...
import com.batz.composite.repository.CompositeRepository;
...
public class CompositeResourceIT {
    ...
    @Mock
    private CompositeRepository compositeRepositoryMock;
    ...
    public void getAllCompositesWithEagerRelationshipsIsEnabled() throws Exception {
        CompositeResource compositeResource = new CompositeResource(compositeRepositoryMock);
        when(compositeRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restCompositeMockMvc = MockMvcBuilders.standaloneSetup(compositeResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restCompositeMockMvc.perform(get("/api/composites?eagerload=true"))
        .andExpect(status().isOk());

        verify(compositeRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllCompositesWithEagerRelationshipsIsNotEnabled() throws Exception {
        CompositeResource compositeResource = new CompositeResource(compositeRepositoryMock);
            when(compositeRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restCompositeMockMvc = MockMvcBuilders.standaloneSetup(compositeResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
