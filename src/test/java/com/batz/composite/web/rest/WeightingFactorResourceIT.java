...
import com.batz.composite.CompositeApp;
import com.batz.composite.domain.WeightingFactor;
import com.batz.composite.repository.WeightingFactorRepository;
import com.batz.composite.web.rest.errors.ExceptionTranslator;
...
import static com.batz.composite.web.rest.TestUtil.createFormattingConversionService;
 ...
 */
@SpringBootTest(classes = CompositeApp.class)
public class WeightingFactorResourceIT {
    ...
    @Mock
    private WeightingFactorRepository weightingFactorRepositoryMock;
    ...
    public void getAllWeightingFactorsWithEagerRelationshipsIsEnabled() throws Exception {
        WeightingFactorResource weightingFactorResource = new WeightingFactorResource(weightingFactorRepositoryMock);
        when(weightingFactorRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restWeightingFactorMockMvc = MockMvcBuilders.standaloneSetup(weightingFactorResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restWeightingFactorMockMvc.perform(get("/api/weighting-factors?eagerload=true"))
        .andExpect(status().isOk());

        verify(weightingFactorRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllWeightingFactorsWithEagerRelationshipsIsNotEnabled() throws Exception {
        WeightingFactorResource weightingFactorResource = new WeightingFactorResource(weightingFactorRepositoryMock);
            when(weightingFactorRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restWeightingFactorMockMvc = MockMvcBuilders.standaloneSetup(weightingFactorResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
