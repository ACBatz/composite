...
import com.batz.composite.repository.UnitOfMeasureRepository;
...
public class UnitOfMeasureResourceIT {
    ...
    public void getAllUnitOfMeasuresWithEagerRelationshipsIsEnabled() throws Exception {
        UnitOfMeasureResource unitOfMeasureResource = new UnitOfMeasureResource(unitOfMeasureRepositoryMock);
        when(unitOfMeasureRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restUnitOfMeasureMockMvc = MockMvcBuilders.standaloneSetup(unitOfMeasureResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restUnitOfMeasureMockMvc.perform(get("/api/unit-of-measures?eagerload=true"))
        .andExpect(status().isOk());

        verify(unitOfMeasureRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllUnitOfMeasuresWithEagerRelationshipsIsNotEnabled() throws Exception {
        UnitOfMeasureResource unitOfMeasureResource = new UnitOfMeasureResource(unitOfMeasureRepositoryMock);
            when(unitOfMeasureRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restUnitOfMeasureMockMvc = MockMvcBuilders.standaloneSetup(unitOfMeasureResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
