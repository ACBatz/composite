...
import com.batz.composite.repository.CalculationRepository;
...
public class CalculationResourceIT {
    ...
    public void getAllCalculationsWithEagerRelationshipsIsEnabled() throws Exception {
        CalculationResource calculationResource = new CalculationResource(calculationRepositoryMock);
        when(calculationRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restCalculationMockMvc = MockMvcBuilders.standaloneSetup(calculationResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restCalculationMockMvc.perform(get("/api/calculations?eagerload=true"))
        .andExpect(status().isOk());

        verify(calculationRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllCalculationsWithEagerRelationshipsIsNotEnabled() throws Exception {
        CalculationResource calculationResource = new CalculationResource(calculationRepositoryMock);
            when(calculationRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restCalculationMockMvc = MockMvcBuilders.standaloneSetup(calculationResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
