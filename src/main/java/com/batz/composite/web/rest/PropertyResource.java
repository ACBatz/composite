...
/**
 * REST controller for managing {@link com.batz.composite.domain.Property}.
 */
@RestController
@RequestMapping("/api")
public class PropertyResource {
    ...
    public List<Property> getAllProperties(@RequestParam(required = false) String filter) {
        if ("limit-is-null".equals(filter)) {
            log.debug("REST request to get all Propertys where limit is null");
            return StreamSupport
                .stream(propertyRepository.findAll().spliterator(), false)
                .filter(property -> property.getLimit() == null)
