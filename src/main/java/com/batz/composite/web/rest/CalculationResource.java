...
/**
 * REST controller for managing {@link com.batz.composite.domain.Calculation}.
 */
@RestController
@RequestMapping("/api")
public class CalculationResource {
    ...
    public List<Calculation> getAllCalculations(@RequestParam(required = false) String filter,@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        if ("noun-is-null".equals(filter)) {
            log.debug("REST request to get all Calculations where noun is null");
            return StreamSupport
                .stream(calculationRepository.findAll().spliterator(), false)
                .filter(calculation -> calculation.getNoun() == null)
                .collect(Collectors.toList());
        }
        if ("verb-is-null".equals(filter)) {
            log.debug("REST request to get all Calculations where verb is null");
            return StreamSupport
                .stream(calculationRepository.findAll().spliterator(), false)
                .filter(calculation -> calculation.getVerb() == null)
                .collect(Collectors.toList());
        }
        log.debug("REST request to get all Calculations");
        return calculationRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Calculation> getCalculation(@PathVariable Long id) {
        ...
        Optional<Calculation> calculation = calculationRepository.findOneWithEagerRelationships(id);
