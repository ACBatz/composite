...
/**
 * REST controller for managing {@link com.batz.composite.domain.Composite}.
 */
@RestController
@RequestMapping("/api")
public class CompositeResource {
    ...
    public List<Composite> getAllComposites(@RequestParam(required = false) String filter,@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        ...
        return compositeRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Composite> getComposite(@PathVariable Long id) {
        ...
        Optional<Composite> composite = compositeRepository.findOneWithEagerRelationships(id);
