...
/**
 * REST controller for managing {@link com.batz.composite.domain.LoadDirection}.
 */
@RestController
@RequestMapping("/api")
public class LoadDirectionResource {
    ...
    public List<LoadDirection> getAllLoadDirections() {
        log.debug("REST request to get all LoadDirections");
        return loadDirectionRepository.findAll();
    }
    ...
    public ResponseEntity<LoadDirection> getLoadDirection(@PathVariable Long id) {
        ...
        Optional<LoadDirection> loadDirection = loadDirectionRepository.findById(id);
